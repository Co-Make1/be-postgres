...
router.post("/", restricted, async (req, res, next) => {
  try {
    ...
    const valCheck = validator.filter(
      issue => issue.issue_id === upvote.issue_id && issue
    );
    ...
    if (valCheck.length > 0) {
      return res
        .status(400)
        .json({
          message: `User with the id of ${upvote.user_id} has already upvoted issue with id of ${upvote.issue_id}`
        });
    }
